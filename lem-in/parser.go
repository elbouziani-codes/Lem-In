	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(trimmedLine, "#") {
			if trimmedLine == "##start" {
				if step == 2 || isStartNext || isEndNext{
					return nil, "", ErrInvalidFormat // Coordinates cannot come after links
				}
				isStartNext = true
			} else if trimmedLine == "##end" {
				if step == 2 || isEndNext || isStartNext{
		...
		case 0:
			...
			ants, err := strconv.Atoi(trimmedLine)
			if err != nil || ants <= 0 {
				return nil, "", ErrInvalidNumAnts
			}
			graph.NumAnts = ants
			step = 1
		case 1:
			...
			if strings.Contains(trimmedLine, "-") && !strings.Contains(trimmedLine, " ") {
				// Transition to links
				step = 2
				err := parseLink(graph, trimmedLine)
				if err != nil {
					return nil, "", err
				}
			} else {
				...
				err := parseRoom(graph, trimmedLine, isStartNext, isEndNext)
				if err != nil {
					return nil, "", err
				}
				isStartNext = false
